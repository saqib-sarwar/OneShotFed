...
filename = "ivon_one_shot_global_adam_"+str(seed)+"_"+dataset+"_"+model_name+"_"+"_"+str(local_epochs)
...
def objective(trial):
    ...
    eta = trial.suggest_categorical('eta', [0.005, 0.01, 0.02, 0.05, 0.1])
    ess = trial.suggest_categorical('ess', [5000, 10000, 20000, 50000])
    hess_init = trial.suggest_categorical('hess_init', [0.001, 0.005, 0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5])
    bs = trial.suggest_categorical('bs', [64, 128])
    local_epochs = trial.suggest_categorical('local_epochs', [10, 20, 30, 40, 50])
    ...
    # }
        ...
        for t in range(0,args['rounds']):
            ...
            for i in ind:
                ...
                model_vector, model, client_hess =local.train_ivon(copy.deepcopy(net_glob), args['n_c'])
                model_vectors.append(model_vector)
                models.append(model)
                loc_test_acc, loc_test_loss = test_img(model, dataset_test_global,args)
                print ("Local Model ", i, "Test Acc. ", loc_test_acc, "Test Loss ", loc_test_loss)
        ...
        dict_results[alg +'local_model_test_accuracies_' + str(seed)+'_alpha_'+str(alpha)+"_eta_"+str(eta)+ '_ess_'
                     +str(ess)+'_h_init_'+str(hess_init)+'_bs_'+str(bs)+'_l_epoch_'+str(local_epochs)] = local_model_accs
        dict_results[alg +'local_model_test_losses_' + str(seed)+'_alpha_'+str(alpha)+"_eta_"+str(eta)+ '_ess_'
                     +str(ess)+'_h_init_'+str(hess_init)+'_bs_'+str(bs)+'_l_epoch_'+str(local_epochs)] = local_model_loss
        ...
        net_glob, global_hessian = get_one_shot_model(alg, d,n,p,args,net_glob, models, model_vectors, \
        ...
        dict_results[alg + '_test_loss_'+'seed_' + str(seed)+"_eta_"+str(eta)+ '_ess_'
                     +str(ess)+'_h_init_'+str(hess_init)+'_bs_'+str(bs)+'_l_epoch_'+str(local_epochs)] = test_loss
        dict_results[alg + '_test_acc_'+'seed_' + str(seed)+"_eta_"+str(eta)+ '_ess_'
                     +str(ess)+'_h_init_'+str(hess_init)+'_bs_'+str(bs)+'_l_epoch_'+str(local_epochs)] = test_acc
